...
def init_args():
    ...
    output_dir = os.path.join("/opt/ml/output",f"{args.paradigm}")
...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        print ("len(self.hparams.n_gpu)",len(self.hparams.n_gpu))
...
if __name__ == '__main__':
    # initialization
    args = init_args()
    env_hyper = env['hyperparameters']
    #print type
    print ("type: ", type(env_hyper))
    
    args.task = env_hyper["task"]
    args.dataset = env_hyper["dataset"]
    args.model_name_or_path = env_hyper["model_name_or_path"]
    args.paradigm = env_hyper["paradigm"]
    args.eval_batch_size = int(env_hyper["eval_batch_size"])
    args.train_batch_size = int(env_hyper["train_batch_size"])
    args.learning_rate = float(env_hyper["learning_rate"])
    args.num_train_epochs = int(env_hyper["num_train_epochs"])
    args.do_train = env_hyper["do_train"]
    args.do_eval = env_hyper["do_eval"]
    args.do_direct_eval = env_hyper["do_direct_eval"]
       
    print ("args task: ",args.task)
    print ("env task: ", env_hyper['task'])
