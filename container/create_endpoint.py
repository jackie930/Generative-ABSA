...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="create SageMaker container"
    )
    parser.add_argument(
        "-e",
...
def is_endpoint_running(endpoint_name):
    ...
    if the named container doesn't exist then return None.
    """
...
def deploy_endpoint():
    ...
    try:
        ...
        print("!!! Cannot create container - Exception is >> {}".format(e))
        if type(e).__name__ == "StateMachineAlreadyExists":
            print("Skip sf creation because it is created before.")
        else:
            raise e

    print("<<< Completed model container deployment. " + str(args.endpoint_name))
