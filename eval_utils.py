...
def compute_scores(pred_seqs, gold_seqs, sents, io_format, task):
    ...
    #for i in range(10):
        if io_format == 'annotation':
            ...
            gold_list,gold_extractions_with_aspect = extract_spans_extraction(task, gold_seqs[i])
            pred_list,pred_extractions_with_aspect = extract_spans_extraction(task, pred_seqs[i])
